    ...
    if(node == null){//not found
      return null;
        ...
        //if n has two children
          ...
          BinaryTreeNode min = min(node.rightChild);

          /*copy min entry to node entry*/
          node.entry = min.entry;

          /*removing min*/
          if(node.rightChild == min) // if min is directly the right child of to be removed node
          {                          // and min has a right child
            node.rightChild = min.rightChild;
          }
          else
          {
            if(min.rightChild == null) // if min has no right child , min is a leaf
            {
              min.parent.leftChild = null;
            }
            else // if min has a right child
            {
